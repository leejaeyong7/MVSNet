...
def train(dataset_dir):
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        dataset_dir, num_neighbors, depth_interval, image_width, image_height, mode
        dataset = MVSDataset(dataset_dir, 5, FLAGS.max_d, FLAGS.max_w, FLAGS.max_h, 'train')
        training_sample_size = len(dataset)
        ########## data iterator #########
        # training generators
        training_generator = iter(dataset)
...
def main(argv=None):  # pylint: disable=unused-argument
    """ program entrance """
    # Training entrance.
    train('/mnt/data/datasets/tnt/sets/')
